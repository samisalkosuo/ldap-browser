...
async def get_root_dse(connection_id: str):
...
async def get_children(connection_id: str, dn: Optional[str] = Query(None)):
...
async def get_entry(connection_id: str, dn: str = Query(...)):
...
async def search_ldap(connection_id: str, request: SearchRequest):
    """Perform LDAP search."""
    client = connection_manager.get_connection(connection_id)
    if not client:
        raise HTTPException(status_code=404, detail="Connection not found")
